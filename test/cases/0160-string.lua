...
test "escape_for_json-injection" (function ()
  ensure_strequals("common json injection",
    escape_for_json(
      ...
      "88,83,83))//\";alert(String.fromCharCode(88,83,83))//\";alert(String." ..
      "fromCharCode(88,83,83))//--></SCRIPT>\">'><SCRIPT>alert(String.fromCh" ..
      "arCode(88,83,83))</SCRIPT>"
    ),
    "\"';alert(String.fromCharCode(88,83,83))//';alert(String.fromCharCode(8" ..
    "8,83,83))//\\\";alert(String.fromCharCode(88,83,83))//\\\";alert(String" ..
    ".fromCharCode(88,83,83))//--></SCRIPT>\\\">'><SCRIPT>alert(String.fromC" ..
...
test:test "split_by_offset-basic" (function ()
  ...
  local st = "Lorem ipsum dolor sit amet"
  ensure_fails_with_substring(
  ...
  ensure_returns("offset < 0", 2, { st, st }, split_by_offset(st,-1))
  ensure_returns("offset = 0", 2, { "", st }, split_by_offset(st,0))
  ensure_returns("offset = 1",
    2, { "L", "orem ipsum dolor sit amet" },
    split_by_offset(st,1)
   )
  ensure_returns("max offset", 2, { st, "" }, split_by_offset(st, #st))
  ensure_returns("max offset and skip 0",
    2,
    { st, "" },
    split_by_offset(st, #st, 0)
   )
  ensure_returns("max offset and skip -2",
    2,
    { st, "et" },
    split_by_offset(st, #st, -2)
   )
  ensure_returns("max offset and skip -max",
    2,
    { st, st },
    split_by_offset(st, #st, -(#st))
   )
  ensure_returns("max offset and skip under begin",
    2,
    { st, st },
    split_by_offset(st, #st, -1 - (#st))
   )
  ensure_returns("max offset and skip max",
    2,
    { st, "" },
  ...
  ensure_returns("offset 1 and skip 0",
    2,
    { "L", "orem ipsum dolor sit amet" },
    split_by_offset(st, 1, 0)
   )
  ensure_returns("offset 1 and skip 5",
    2,
    { "L", "ipsum dolor sit amet" },
    split_by_offset(st, 1, 5)
   )
  ensure_returns("offset 5 and skip 5",
    2,
    { "Lorem", "m dolor sit amet" },
    split_by_offset(st, 5, 5)
   )
end)
...
test:test "count_substrings-basic" (function ()
  ...
  ensure_equals("str empty", count_substrings("",str), 0)
  ensure_equals("substr empty", count_substrings(str,""), 0)
  ensure_equals("str equal substr", count_substrings("t", "t"), 1)
  ensure_equals("zero count", count_substrings(str, "est"), 0)
  ensure_equals("positive count", count_substrings(str, "o"), 4)
  ensure_equals("positive count for word", count_substrings(str, "sit"), 1)
  ensure_equals("special character string",
    count_substrings("\nLorem \tipsum?#$%^&*()_+|~/\t \0dolor \007sit.\n", "o"),
    3
   )
end)
