...
test:test_for "fill_placeholders_ex" (function ()
  ...
  ensure_strequals("extra right round brace", fill_placeholders_ex("%$%((.-)%)", "`$(a)')", { a = 42 }), "`42')")
  ensure_strequals("extra right curly brace", fill_placeholders_ex("%$%((.-)%)", "`$(a)'}", { a = 42 }), "`42'}")

  ensure_strequals("curly: both empty", fill_placeholders_ex("%${(.-)}", "", {}), "")
  ensure_strequals("curly: empty dict", fill_placeholders_ex("%${(.-)}", "test", {}), "test")
  ensure_strequals("curly: empty str", fill_placeholders_ex("%${(.-)}", "", { a = 42 }), "")
  ensure_strequals("curly: missing key", fill_placeholders_ex("%${(.-)}", "${b}", { a = 42 }), "${b}")

  ensure_strequals("curly: bad format", fill_placeholders_ex("%${(.-)}", "$a", { a = 42 }), "$a")
  ensure_strequals("curly: missing right brace", fill_placeholders_ex("%${(.-)}", "$a}", { a = 42 }), "$a}")
  ensure_strequals("curly: missing left brace", fill_placeholders_ex("%${(.-)}", "${a", { a = 42 }), "${a")

  ensure_strequals("curly: ok", fill_placeholders_ex("%${(.-)}", "a = `${a}'", { a = 42 }), "a = `42'")
  ensure_tequals("curly: no extra data", { fill_placeholders_ex("%${(.-)}", "a = `${a}'", { a = 42 }) }, { "a = `42'" })

  ensure_strequals("curly: extra key", fill_placeholders_ex("%${(.-)}", "a = `${a}'", { a = 42, b = 43 }), "a = `42'")
  ensure_strequals("curly: two keys", fill_placeholders_ex("%${(.-)}", "`${key}' = `${value}'", { key = "a", value = 42 }), "`a' = `42'")

  ensure_strequals("curly: empty string key", fill_placeholders_ex("%${(.-)}", "`${}'", { [""] = 42 }), "`42'")

  ensure_strequals("curly: extra braces", fill_placeholders_ex("%${(.-)}", "${a `${a}'}", { a = 42 }), "${a `${a}'}")
  ensure_strequals("curly: extra right brace", fill_placeholders_ex("%${(.-)}", "`${a}'}", { a = 42 }), "`42'}")

  ensure_strequals("curly: extra round braces", fill_placeholders_ex("%${(.-)}", "${a `${a}'}", { a = 42 }), "${a `${a}'}")
  ensure_strequals("curly: extra right curly brace", fill_placeholders_ex("%${(.-)}", "`${a}'}", { a = 42 }), "`42'}")
  ensure_strequals("curly: extra right round brace", fill_placeholders_ex("%${(.-)}", "`${a}')", { a = 42 }), "`42')")
end)
